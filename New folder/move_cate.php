...
<script type="text/javascript">
	...
	$(function() {
		...
		.droppable({drop: function (event,div) {
			...
			alert(alerts)
		xmlhttp.open("GET","save_cate.php?code="+id+"&name="+name+"&tonode="+tonode+"&fromnode="+fromnode+"&fromnodename="+fromnodename+"&tonodename="+tonodename,true);
		xmlhttp.send();
...
echo "<option value='' data-value='".$b_result[$b]['thaiName']."'>-- เลือก Category ต้นทาง --</option>";
...
echo "เลือก Category ปลายทาง : <select class='form-control' id='sto' onchange='return to_id(this);'>";
echo "<option value='' data-value='".$result[$t]['thaiName']."'>-- เลือก Category ปลายทาง --</option>";
for($t=0;$t<$cnt;$t++){
