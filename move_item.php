...
<div class="col-md-9 jstreecenter">
	...
	fclose($server1);
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,$urlserver.$urlitems);//http://s01xp.dyndns.org:8080/SmartQWs/pickup/search");
	   ...
	   if($item=="[]"){
	   	echo "<script>
	   	alert('กรุณาเลือก Sub-category Level เพื่อทำการ move Items') 
	   	</script>
	   	<script>
		...
		$all="";
		  ...
		  $result = array_values($result);
		    ...
		    $cntf=count($result);
			   	...
			   	if(empty($_GET['all'])){
			   		$chk1="";
			   		$chk="<a href='move_item.php?cateid=$cateid&sub=$subcateN&all=1&subcate=$_GET[subcate]&search=$_GET[search]'>เลือกทั้งหมด</a>";
			   	}else if($_GET['all']==1){
			   		$chk1="checked='checked'";
			   		$chk="<a href='move_item.php?cateid=$cateid&sub=$subcateN&all=0&subcate=$_GET[subcate]&search=$_GET[search]'>ยกเลิกทั้งหมด</a>";
		   		}
		   		if($_GET['search']){
		   			if($cateid==""){$_GET['subcate']="ไม่มี subcate ต้นทาง";}else{$_GET['subcate']=$_GET['subcate'];}
		   		echo "<h3>Subcate ID ".$_GET['subcate']."<br>ค้นหาตามชื่อ ".$_GET['search']."</h3>";
		   		}else{
	   		...
	   		<form class='form-inline'>
           		<input type='hidden' name='subcate' value="<?php echo $subcateN; ?> "></input>
           		<input type='hidden' name='cateid' value="<?php echo $cateid; ?> "></input>
           		<input type='text' name='search' class='form-control'></input>
           		<button type='submit' class='btn btn-success'> Search </button>
           </form>
           <form action='move.php' method='POST' class="form-inline">
           <?php
            echo "<input type='hidden' name='FromNode' id='FromNode' class='form-control' value='$cateid:$subcateN'>";
	   		...
	   		echo "<tr><th align='center'>$chk</th><th align='center'>รหัสสินค้า</th><th align='center'>รายการ</th></tr>";

		   		for($i=0;$i<$cntf;$i++){
		   			echo "<tr><td align='center'><input type='checkbox' name='item_id[]' value='".$items['itemCode'][$i].";".$items['itemName'][$i]."' $chk1></td><td align='center'>".$items['itemCode'][$i]."</td><td>".$items['itemName'][$i]."</td></tr>";
		   		}
	   		echo "</table>";
